...
class PdfShiftParser:
    ...
    def _detect_day_columns(self, words: Sequence[dict]) -> List[float]:
        ...
        numbered_columns: Dict[int, float] = {}
        for word in words:
            text = word.get("text", "")
            if DAY_PATTERN.fullmatch(text):
                day_num = int(text)
                x_center = (word["x0"] + word["x1"]) / 2
                numbered_columns[day_num] = round(x_center, 2)
            elif text in ["月", "火", "水", "木", "金", "土", "日"]:
                x_center = (word["x0"] + word["x1"]) / 2
                x_positions.append(round(x_center, 2))

        if numbered_columns:
            max_day = max(numbered_columns.keys())
            ordered = [numbered_columns[day] for day in range(1, max_day + 1) if day in numbered_columns]
            return ordered
    ...
    def _find_employee_anchors(self, words: List[dict]) -> List[Dict]:
        """
        6 桁以上の数字のみを社員番号として扱う。

        集計行や凡例に出てくる 1〜2 桁の数字を社員番号と誤認しないよう、
        6 桁未満は無視する。
        """
    ...
    def _cluster_time_status_lines(self, tokens: Sequence[dict]) -> List[float]:
        """時刻/ステータスが載る行の y 座標をクラスタリングして返す。"""
        ...
        y_sorted = sorted(
            [
                (token.get("top", 0.0) + token.get("bottom", 0.0)) / 2
                for token in tokens
                if TIME_PATTERN.search(token.get("text", "").strip())
                or STATUS_PATTERN.search(token.get("text", "").strip())
            ]
        )

        for y_center in y_sorted:
            if not buckets or abs(y_center - buckets[-1]) > LINE_TOLERANCE:
                buckets.append(y_center)
            else:
                buckets[-1] = (buckets[-1] + y_center) / 2
        return buckets
    ...
    def _resolve_entry_exit_lines(self, tokens: Sequence[dict]) -> Tuple[float | None, float | None]:
        ...
        # ヒューリスティック: 時刻/ステータスを含む行を 2 本拾い、入退として使う
        line_candidates = self._cluster_time_status_lines(tokens)

        if entry_y is None and line_candidates:
            entry_y = line_candidates[0]
        if exit_y is None:
            if len(line_candidates) > 1:
                exit_y = line_candidates[1]
